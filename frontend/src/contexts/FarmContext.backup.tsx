...
export function FarmProvider({ children }: { children: ReactNode }) {
  ...
  // Initialize selected farm from localStorage after mounting and after farms are loaded
  useEffect(() => {
    if (typeof window !== 'undefined' && farms.length > 0) {
      // Only try to restore if we don't already have a selected farm
      // This prevents overriding a manually selected farm
      if (!selectedFarm) {
        const savedFarmId = localStorage.getItem('selectedFarmId');
        if (savedFarmId) {
          const farmToSelect = farms.find((farm: Farm) => farm._id === savedFarmId) || null;
          if (farmToSelect) {
